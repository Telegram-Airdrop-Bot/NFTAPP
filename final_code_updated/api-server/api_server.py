...
def get_nft_assets(wallet_address):
    """Get NFT assets for a wallet address using Helius DAS API"""
    try:
        ...
        if not wallet_address or len(wallet_address) < 32 or len(wallet_address) > 44:
            print(f"❌ Invalid wallet address format: {wallet_address} (length: {len(wallet_address) if wallet_address else 0})")
            return jsonify({"error": "Invalid wallet address", "details": "Wallet address format is invalid"}), 400
        
        # Check if wallet address contains only valid characters
        import re
        if not re.match(r'^[A-Za-z0-9]+$', wallet_address):
            print(f"❌ Invalid wallet address characters: {wallet_address}")
            return jsonify({"error": "Invalid wallet address", "details": "Wallet address contains invalid characters"}), 400
            
        print(f"✅ API key validation passed, length: {len(api_key)}")
        print(f"✅ Wallet address validation passed: {wallet_address[:8]}...{wallet_address[-4:]}")
        
        # Use the modern Helius DAS API endpoint for NFTs
        # Based on: https://www.helius.dev/docs/api-reference/das/getasset
        das_url = "https://mainnet.helius-rpc.com"
        
        # Use searchAssets method to get all assets owned by the wallet
        payload = {
            "jsonrpc": "2.0",
            "id": "my-id",
            "method": "searchAssets",
            "params": {
                ...
                "tokenType": "all",  # Get both NFTs and tokens
                "displayOptions": {
        ...
        print(f"🌐 Calling Helius DAS API: {das_url}")
        ...
        print(f"📊 Helius DAS API response status: {response.status_code}")
        ...
        if response.status_code == 200:
            ...
            if "error" in data:
                error_msg = f"Helius DAS API error: {data['error']}"
                print(f"❌ {error_msg}")
                return jsonify({"error": "Failed to fetch NFTs", "details": error_msg}), 400
            
            # Extract assets from the response
            if "result" in data and "items" in data["result"]:
                assets = data["result"]["items"]
                
                # Filter for NFTs only (exclude regular tokens)
                nfts = []
                for asset in assets:
                    # Check if it's an NFT based on token standard
                    token_standard = asset.get("content", {}).get("metadata", {}).get("token_standard", "")
                    interface = asset.get("interface", "")
                    
                    # Consider it an NFT if it has NFT-like properties
                    if (token_standard in ["NonFungible", "ProgrammableNonFungible"] or 
                        interface in ["V1_NFT", "MplCoreAsset"] or
                        asset.get("content", {}).get("files") or
                        asset.get("content", {}).get("metadata", {}).get("name")):
                        nfts.append(asset)
                
                print(f"✅ Successfully fetched {len(nfts)} NFTs from Helius DAS API")
                result = jsonify(nfts)
                ...
                return jsonify({"error": "Unexpected API response format", "details": "Invalid response structure"}), 500
        else:
            error_msg = f"Helius DAS API returned status {response.status_code}"
            try:
            ...
            print(f"❌ Helius DAS API error: {error_msg}")
            return jsonify({"error": "Failed to fetch NFTs", "details": error_msg}), response.status_code
            
    except requests.exceptions.Timeout:
        print("❌ Helius DAS API request timed out")
        return jsonify({"error": "Request timeout", "details": "Helius DAS API request timed out"}), 408
    except requests.exceptions.RequestException as e:
        print(f"❌ Helius DAS API request failed: {e}")
