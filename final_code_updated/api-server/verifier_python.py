...
def get_wallet_nfts_by_collection(wallet_address: str, collection_id: str = None) -> Optional[List[Dict]]:
    ...
    # Using Helius DAS API for NFTs - modern approach
    ...
    try:
        ...
        if "result" in data and "items" in data["result"]:
            ...
            # Improved NFT filtering using DAS API data
            nfts = []
            for item in all_items:
                # Check if it's an NFT based on multiple criteria from DAS API
                is_nft = False
                
                # Criterion 1: Token standard from DAS API
                token_standard = item.get("content", {}).get("metadata", {}).get("token_standard", "")
                if token_standard in ["NonFungible", "ProgrammableNonFungible"]:
                    is_nft = True
                
                # Criterion 2: Interface type from DAS API
                interface = item.get("interface", "")
                if interface in ["V1_NFT", "MplCoreAsset"]:
                    is_nft = True
                
                # Criterion 3: Has files or name/symbol (NFT characteristics)
            ...
            # If collection_id is specified, filter by collection using DAS API grouping
            if collection_id:
                filtered_nfts = []
                for nft in nfts:
            ...
            print(f"❌ No 'result' or 'items' in DAS API response")
            return []
    except requests.RequestException as e:
        print(f"❌ Error fetching NFTs from DAS API: {e}")
        return None
